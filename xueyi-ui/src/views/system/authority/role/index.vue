  ...
  <div class="app-container">
    ...
    <div class="wrapper-container">
      <el-row :gutter="10" class="mb8">
        ...
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['authority:role:edit']"
          >修改
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['authority:role:delete']"
          >删除
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            size="mini"
            @click="handleExport"
            v-hasPermi="['authority:role:export']"
          >导出
          </el-button>
        </el-col>
      ...
      <el-table v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="角色编码" prop="code" min-width="120" align="center"/>
        ...
        <el-table-column label="操作" align="center" min-width="120" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-hasPermi="['authority:role:auth']"
            >菜单配置
            </el-button>
            <el-button
              size="mini"
        ...
        <el-row>
          ...
          <span slot="label">
            ...
            权限字符
              ...
              <el-input v-model="form.roleKey" placeholder="请输入权限字符" :readonly="form.id !== undefined"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <el-dialog :title="title" :visible.sync="openMenuScope" width="500px" append-to-body v-dialogDrag
      ...
      <el-form ref="form" :model="form" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="角色编码" prop="code">
              <el-input v-model="form.code" placeholder="请输入角色编码" readonly/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="角色名称" prop="name">
...
import {STATUS} from "@/constants/BaseConstants"
import {DATA_SCOPE} from "@/constants/AuthContants"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listRoleApi(this.addDateRange(this.queryParams, this.dateRange)).then(
        response => {
    ...
    /** 根据角色Id查询模块&菜单树结构 */
    getMenuScope(id) {
      return getRoleMenuRange({id: id}).then(response => {
        return response
      })
    },
    /** 根据角色Id查询部门-岗位树结构 */
    getDataScope(id) {
      return getRoleDataRange({id: id}).then(response => {
        return response
      })
    },
    /** 角色状态修改 */
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.name + '"角色吗?').then(function () {
        return editStatusRoleApi(row.id, row.status)
      }).then(() => {
    ...
    handleUpdate(row) {
      ...
      getRoleApi({id: id}).then(response => {
    ...
    handleMenuScope(row) {
      ...
      const menuScope = this.getMenuScope(row.id)
      getRoleApi({id: row.id}).then(response => {
        this.form = response.data
        menuScope.then(res => {
    ...
    handleDataScope(row) {
      ...
      const dataScope = this.getDataScope(row.id)
      getRoleApi({id: row.id}).then(response => {
        this.form = response.data
